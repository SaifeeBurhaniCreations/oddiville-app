...
function mapOrderProducts(order) {
  ...
  for (const [productId, skuMap] of Object.entries(
    order.dispatched_items ?? {},
  )) {
    const product = result.get(productId);
    if (!product) continue;

    for (const [skuKey, skuData] of Object.entries(skuMap)) {
      const [sizeStr, unit, ratingStr] = skuKey.split("-");
...
function getTotalQuantity(products = []) {
  return products.reduce((total, product) => {
    const chamberQty =
      product.chambers?.reduce((sum, c) => sum + Number(c.quantity || 0), 0) ||
      0;
...
function getProductDetailsSection(order, productDetails, totalQuantity, actor) {
  return [
    {
      row_1: [
        {
...
function buildRawMaterialSchemas({ rmOrder, vendors, currentUser }) {
...
function buildProductionSchemas({ production = {}, lane = {}, rmOrder = {} }) {
  ...
  return {
    "production-start": {
      ...
      fileName: rmOrder?.truck_details?.challan?.url
        ? [rmOrder.truck_details.challan.url]
        : ["challan.png"],
    },
  ...
  return {
    "worker-multiple": {
      ...
      table: contractors.map((contractor) => ({
        ...
        tableHeader: [
          { label: "Locations", key: "location", flex: 3 },
...
function buildCalendarSchemas({ calendarEvent }) {
  ...
  const title = format(new Date(calendarEvent.scheduled_date), "MMM d, yyyy");
