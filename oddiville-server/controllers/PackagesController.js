...
const { Packages, DryWarehouse: DryWarehouseClient, Chambers: ChambersClient, sequelize } = require("../models");
...
router.post('/create', async (req, res) => {
    try {
      ...
      if (!chamber_name || typeof chamber_name !== 'string') {
        return res.status(400).json({ error: "chamber_name is required and must be a string." });
      }
      ...
      types = types.map((item) => {
        ...
        return {
          size: typeof item.size === 'string' ? item.size.trim() : String(item.size ?? ''),
          quantity: typeof item.quantity === 'string' ? item.quantity.trim() : String(item.quantity ?? '0'),
      ...
      for (const item of types) {
        if (typeof item.size !== 'string' || item.size.length === 0) {
          return res.status(400).json({ error: "Each type must have a non-empty 'size' string." });
        }
        if (typeof item.quantity !== 'string') {
          return res.status(400).json({ error: "Each type must have 'quantity' as a string." });
        }
        if (!(item.unit === null || (typeof item.unit === 'string' && ['kg', 'gm', 'null'].includes(item.unit)))) {
          return res.status(400).json({ error: "Unit must be 'kg', 'gm', 'null' as a string, or actual null." });
        }
      }
  
      product_name = product_name.trim();
      raw_materials = raw_materials.map((v) => (typeof v === 'string' ? v.trim() : v));
      chamber_name = chamber_name.trim();
 
      if (!sequelize) {
        return res.status(500).json({ error: "Database transaction unavailable." });
      }
...
router.patch('/update/:id', async (req, res) => {
    // await DryWarehouseClient.update({ quantity: existingItem.quantity + quantityNum }, { where: { id: existingItem.id }, transaction: t });
    try {
        const { id } = req.params;
