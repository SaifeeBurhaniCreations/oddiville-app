...
const ChamberScreen = () => {
  const { data: chambersData = [], isFetching: chambersLoading, refetch: refetchChambers } = useChamber();
  const { data, isFetching: stockLoading, refetch: refetchStocks } = useChamberStock();
  ...
  const parsedChambers = useMemo(() => {
    ...
    const summaryMap = new Map<string, { totalQuantity: number }>(
      (summaries ?? []).map(s => [String(s.chamberId), { totalQuantity: s.totalQuantity }])
    );
  
    return chambersData.map((chamber: { id: string; chamber_name: string; capacity: number; tag?: string }) => {
      const chamberId = chamber.id;
  
      const relatedStocks = stockData?.filter((stockItem: { chamber: { id: string }[] }) =>
        stockItem.chamber?.some((c) => c.id === chamberId)
      );
  
      const filledFromStocks = (relatedStocks ?? []).reduce((sum: number, stockItem: StockDataItem) => {
        const chamberSum = (stockItem.chamber ?? [])
          .filter((c: ChamberEntry) => c.id === chamberId)
          .reduce((acc, c) => acc + Number(parseFloat(c.quantity || "0") || 0), 0);
        return sum + chamberSum;
      }, 0);
  
      const summary = summaryMap.get(String(chamberId));
      const filledFromSummary = summary ? Number(summary.totalQuantity || 0) : null;
  ...
  return (
    <View style={styles.rootContainer}>
      ...
      <View style={styles.wrapper}>
        <Tabs tabTitles={['Overview', 'Detail']} color='green' style={[styles.flexGrow, { paddingTop: 16, flexDirection: "column", gap: 24 }]}>
          <View style={styles.flexGrow}>
            <View style={[styles.flexGrow, styles.VStack]}>
              ...
              <FlatList
                data={parsedChambers}
                renderItem={({ item }) => (
                  <ChamberCard filled={item.filled} capacity={item.capacity} name={item.name} />
                )}
