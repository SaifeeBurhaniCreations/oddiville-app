...
import { KeyboardAvoidingView, Platform, ScrollView, TouchableWithoutFeedback, Keyboard, StyleSheet, View, Pressable } from 'react-native';
...
import { B3, B4, BoldHeading, H2 } from '../src/components/typography/Typography';
...
import loginPhoneSchema, { LoginPhoneFormData } from '../src/schemas/LoginPhoneSchema';
...
const LoginScreen = () => {
    ...
    const [checkOtpState, setcheckOtpState] = useState<"error" | "default" | "success">("default")
    ...
    const onSubmitEmail: SubmitHandler<LoginFormData> = async (data: any) => {
        try {
            ...
            if (response.status === 200) {
                ...
                await login(authData.role);

                await SecureStorage.setItemAsync("newsync", JSON.stringify(authData));
                dispatch(setAdmin(authData));

                showToast("success", "Login successfully!");
                if(authData.role === "superadmin" || authData.role === "admin") goTo('admin-home');
                else if(authData.role === "supervisor") goTo('supervisor-raw-material');
            }
        } catch (error: any) {
    ...
    const onSubmitPhone: SubmitHandler<LoginPhoneFormData> = async (data: any) => {
        try {
            setLoading(true);
                    ...
                    >
                        <View style={styles.loginWrapper}>
                            <View style={styles.headerContent}>
                                ...
                                <Alert color={'blue'} text="If you're a new user, please log in with your phone number. If you're an existing user, please log in with your email." />
                            </View>
                                        ...
                                        content: (
                                            <View style={styles.tabContainer} key="email-tab">
                                                <View style={styles.textContainer}>
                                                    <View>
                                                        <H2 style={{ fontFamily: 'FunnelSans-SemiBold' }}>Hey mate,</H2>
                                    ...
                                    },
                                    {
                                        ...
                                        content: !showOtpInput ? (
                                            ...
                                            <View style={styles.tabContainer} key="phone-tab">
                                                <View style={styles.textContainer}>
                                                    <View>
                                                        <H2 style={{ fontFamily: 'FunnelSans-SemiBold' }}>Enter OTP</H2>
                                                    </View>
                                                    <View style={styles.otpDesc}>
                                                        <B4 color={getColor('green', 400)}>
                                                            We are sent a 4-digit code to <B4 color={getColor('green', 700)}>
                                                                +91***90
                                                            </B4>
                                                        </B4>
                                                ...
                                                <View style={styles.otpWithErrorContainer}>
                                                            ...
                                                            <Controller
                                                                ...
                                                                render={({ field: { onChange, onBlur, value } }) => (
                                                                    <Input
                                                                        ...
                                                                        maxLength={1}
                                                                        inputStyle={{ height: 60, textAlign: "center" }}
                                                    ...
                                                    {checkOtpState === "error" && (
                                                        <B3 color={getColor("red", 700)}>Sorry, the OTP you entered is invalid.</B3>
                                                    )}
                                                </View>

                                                <View style={styles.expireTime}>
                                                    <B4 color={getColor("green", 400)}>Don’t receive code?</B4>
                                                    {secondsLeft > 0 ? (
                                                        <B3 color={getColor("green")}>  {`${String(Math.floor(secondsLeft / 60)).padStart(2, '0')}:${String(secondsLeft % 60).padStart(2, '0')}`}</B3>
                                                    ) : (
