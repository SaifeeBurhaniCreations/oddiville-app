...
import { ALL_KEY, PENDING_KEY, useRawMaterialOrderById } from '@/src/hooks/rawMaterialOrders';
...
const SupervisorRawMaterialDetailsScreen = () => {
    ...
    const { rmId } = useParams("supervisor-rm-details", "rmId");
    ...
    const { data: orderData, isFetching, error } = useRawMaterialOrderById(safeRmId);
    ...
    const {
        ...
        {
            arrival_date: [{ type: 'required', message: 'Arrival date is required!' }],
            truck_weight: [
            ...
            quantity_received: [
                ...
                {
                    type: 'custom',
                    validate: (value) => parseWeight(value) > 0 && parseWeight(value) <= Number(orderData?.quantity_ordered),
                    message: 'Quantity must be greater than 0 or smaller then ordered quantity'
                }
            ],
            ...
            truck_number: [
                ...
                { type: 'length', length: 8, message: 'Truck Number must be 8 digit!' },
            ],
            driver_name: [
    ...
    const hasChanges = useMemo(() => {
        ...
        return values.arrival_date !== '' ||
            ...
            values.driver_name !== '' ||
            values.challan !== null;
    }, [values.arrival_date, values.truck_weight, values.tare_weight, values.quantity_received, values.truck_number, values.driver_name, values.challan]);
    ...
    const validateWeights = (): boolean => {
        ...
        if (truckWeight <= tareWeight) {
            showToast("error", "Truck gross weight must be greater than tare weight!");
            return false;
        }

        const netWeight = truckWeight - tareWeight;
        if (Math.abs(netWeight - quantityReceived) > quantityReceived * 0.1) {
            showToast("error", `Net weight (${netWeight.toFixed(2)} kg) differs significantly from quantity received (${quantityReceived} kg). Please verify the weights.`);
    ...
    const onSubmit = async (skipValidation = false) => {
        ...
        try {
            ...
            formData.append("quantity_received", parseWeight(result.data.quantity_received).toString());

            // Start: CORRECT AND SAFE CHALLAN LOGIC
            ...
            if (isChallanObject(challan)) {
                ...
                if (typeof result.data.challan === "string") {
                    const filename = result.data.challan.split("/").pop() || "challan.jpg";
                    ...
                    const fileType = result.data.challan.type ||
            ...
            updateRawMaterialOrder.mutate(
              { id: safeRmId, data: formData },
              {
                onSuccess: (updated) => {
                  showToast(
                    "success",
                    "Raw material order updated successfully!"
                  );
    ...
    const shouldShowTruckDetails = useMemo(() => {
        return values.truck_weight !== '' &&
            values.tare_weight !== '' &&
            parseWeight(values.truck_weight) > 0 &&
            parseWeight(values.tare_weight) >= 0 &&
            isWeightLogicValid &&
            netWeightKg > 0;
    }, [values.truck_weight, values.tare_weight, isWeightLogicValid, netWeightKg]);
        ...
        return (
            <View style={styles.pageContainer}>
                ...
                <View style={styles.wrapper}>
                    <BackButton
                        label='Details'
                        backRoute='supervisor-raw-material'
                    />
                    <View style={styles.errorContainer}>
                        ...
                        <Button
                            variant='outline'
                            onPress={() => goTo('supervisor-raw-material')}
                        >
                            Go Back
                        </Button>
                    </View>
        ...
        <View style={styles.pageContainer}>
                ...
                >
                    ...
                    >
                        <View style={styles.innerWrapper}>
                            ...
                            {!isWeightLogicValid && values.truck_weight && values.tare_weight && (
                            ...
                            <View style={styles.titleWithDataInputs}>
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, quantity_received: true }))}
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, arrival_date: true }))}
                                ...
                                {/* <FormField name="arrival_date" form={{ values, setField, errors }}>
                  {({ value, onChange, error }) => (
                    <Input
                      value={value}
                    ...
                    >
                      Arrival date
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, truck_weight: true }))}
                                ...
                                {/* <FormField name="truck_weight" form={{ values, setField, errors }}>
                  {({ value, onChange, error }) => (
                    <Input
                      value={value}
                      onChangeText={onChange}
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, tare_weight: true }))}
                                            ...
                                            tooltipText={{
                                                title: "Tare Weight",
                                                description: "Net Weight(empty truck) = Gross Weight - Tare Weight"
                                            }}
                                            showTooltip
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, truck_number: true }))}
                                ...
                                >
                                    {({ value, onChange, error }) => (
                                        <Input
                                            ...
                                            onBlur={() => setTouched(t => ({ ...t, driver_name: true }))}
                  ...
                  {({ value, onChange, error }) => (
                    ...
                    <View style={styles.buttonContainer}>
                        <Button
                            variant='fill'
                            disabled={!isValid || loading || updateRawMaterialOrder.isPending || isEditable || !isWeightLogicValid}
                            onPress={() => onSubmit()}
                        >
                            {loading || updateRawMaterialOrder.isPending ? 'Processing...' : 'Confirm Arrival'}
                        </Button>
                    </View>
                </KeyboardAvoidingView>
            </View>
            ...
            <Modal
                ...
                modalData={{
                    ...
                    description: "Are you sure you want to discard all the changes? This action cannot be undone.",
                    buttons: [
                        {
                            label: "Cancel",
                            variant: "outline",
