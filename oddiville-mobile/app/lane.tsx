...
const LaneCard: React.FC<{ lane: any; laneNumber: number }> = React.memo(
  ({ lane, laneNumber }) => {
    const { data: productionData, isLoading: isProductionLoading } =
      useProductionById(lane.production_id);

    const { isOccupied, productName, badgeText, description, image } =
      useMemo(() => {
...
const LaneScreen: React.FC = () => {
  ...
  const sortedLanes = useMemo(() => {
    ...
    const maxLane: number =
      laneNumbers.length > 0 ? Math.max(...laneNumbers) : 0;
    ...
    <View style={styles.pageContainer}>
      ...
      >
        ...
        <View style={[styles.VStack, styles.gap16]}>
          <BackButton label="Lanes" backRoute="production" />
