...
const SupervisorOrderDetailsCard = ({ order, style, ...props }: SupervisorOrderCardProps) => {
            ...
            <View style={styles.card}>
                <View style={styles.topSection}>
                    <View style={styles.titleRow}>
                        ...
                        <View style={styles.titleColumn}>
                            ...
                            {Array.isArray(order.description) && order.description?.length > 0 && order?.description?.map((desc, idx) => (
                                <KeyValueRow key={idx} icon={desc.icon} name={desc.name} value={desc.value} />
                            ))}
                    ...
                    {order.rating && <Tag color="blue" icon={<StarIcon size={12} />}>{order.rating}</Tag>}
                </View>

                {Array.isArray(order.details) && order.details?.length > 0 && order.details.map((detail, idx) => (
                ...
                {(order.name || order.address) && (
                    <View>
                        ...
                        {order.address && <C1 color={getColor("green", 400)}>{order.address} Bijju bhaiya</C1>}
                    </View>
                )}
                ...
                <View style={styles.helperSection}>
                    {order.helperDetails.map((h, idx) => (
                        <React.Fragment key={idx}>
                            ...
                            {order.helperDetails && idx < order.helperDetails?.length - 1 && <View style={styles.helperDivider} />}
                        </React.Fragment>
                    ))}
                </View>
            )}
                ...
                <View style={styles.dispatchSection}>
                    {order.dispatchDetails.map((h, idx) => (
                        <React.Fragment key={idx}>
                            ...
                            {order.dispatchDetails && idx < order.dispatchDetails?.length - 1 && (
                                <View style={[styles.dispatchDateLayout]}>
                                    <View style={[styles.dispatchDateHyphen]} />
                                    <View style={[styles.dispatchDateDifferencer]}>
                                        <SubHeadingV3 color={getColor('light', 100)}>{order.dateDifference}</SubHeadingV3>
                                    </View>
                                    <View style={[styles.dispatchDateHyphen]} />
                                </View>
                            )}
