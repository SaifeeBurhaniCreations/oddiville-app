...
const totalCount = useMemo(() => {
  if (!getRowTotal) return 0

  return content.reduce(
    (acc, row) => acc + getRowTotal(row),
    0
  )
}, [content, getRowTotal])
...
<View
  ...
  style={[
    styles.headerCell,
    col.flex !== undefined ? { flex: col.flex } : undefined,
    ['countMale', 'countFemale'].includes(col.key) && { alignItems: 'center' }
  ]}
>
        <H6>{col.label}</H6>
      </View>
...
<View
  ...
  style={[
    styles.mergedCell,
    columns[colIndex]?.flex !== undefined
...
<View
  ...
  style={[
    ...
    ['countMale', 'countFemale'].includes(col.key) && {
  alignItems: 'center',
}
  ]}
>
...
>
  <View style={[styles.tableWithLabel, { width: "100%" }]}>
