...
const ChipGroup = ({
  ...
  disabledValues = [],
  ...props
}: ChipGroupProps & { activeValue?: any, onChange?: (val: any) => void, disabledValues?: any[] }) => {
  ...
  const handleChipPress = (index: number) => {
    if (type === "radio" && onChange) {
      ...
      // Do NOT touch activeIndices in radio mode
    } else {
  ...
  const isSm = size === "sm";
  return (
    <View style={[styles.chipContainer, isSm ? styles.gap8 : styles.gap12]} {...props}>
      {children && (isSm ? <B5>{children}</B5> : <H4>{children}</H4>)}
      <View style={[styles.container, style]}>
        {data?.map((item, index) => {
          const value = item.value ?? item;
          const isActive = type === "radio"
            ? (activeValue === value)
          ...
          const commonProps = {
            isActive,
            onPress: isDisabled ? undefined : () => handleChipPress(index),
            disabled: isDisabled,
          };
          ...
          const extraProps =
            type === "radio"
              ? {}
              : { dismisible, textSize: size, size, isClickable, onDismiss, index };
            ...
            <ChipComponent key={index} {...commonProps} {...extraProps}>
              {item?.icon ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  {item.icon}
                  {isSm ? (
                    <C1 color={getColor("green", 700)}>{item.title ?? item.name ?? item}</C1>
                  ) : (
