...
import { selectUnit } from '@/src/redux/slices/unit-select.slice';
  ...
  const ManageActionComponent: React.FC<ManageActionProps> = ({ data, color }) => {
    ...
    const packageTypeProduction = useSelector((state: RootState) => state.packageTypeProduction.selectedPackageType);
      const { data: packageData, isFetching: packageLoading } = usePackageById(currentProductId ?? null);
    ...
    const supervisorProduction = {
      sections: [
          ...
          ...selectedChambers.map(chamberName => {
            const quantityValue = chamberQuantity[chamberName]
              ?.find((chamber: ChamberQty) => chamber.name === chamberName)
              ?.quantity ?? ""; 
            ...
            onPress={() => {
              dispatch(selectUnit(action.actionKey));
                ...
                if(source === "add-product-package") {
                  ...
                  validateAndSetData(`${currentProductId}:${packageData.product_name}`, "add-package")
                }

              // const handler = actionHandlers[action.actionKey as ActionKey];
              // if (handler) handler();
            }}
            style={{ borderBottomWidth: index !== data?.length - 1 ? 1 : 0, borderBottomColor: getColor("green", 100), paddingBottom: index !== data?.length - 1 ? 12 : 0 }}
          >
