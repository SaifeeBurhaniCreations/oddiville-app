...
const TitleWithDetailsCrossComponent: React.FC<TitleWithDetailsCrossProps> = ({ data, onClose }) => {
    const dispatch = useDispatch();
    const isEdit = useSelector((state: RootState) => state.packageSizePackaging.isEdit);
            ...
            <View style={styles.titleWithDetailsCross}>
                ...
                <View style={styles.DetailsWithCross}>
                    {details && !isEdit && (
                        <React.Fragment>
                            <View style={[styles.HStack, styles.gap4]}>
                                {details?.icon === "database" ? <DatabaseIcon /> : <Pressable onPress={() => dispatch(setIsPackageEdit(true))}><PencilIcon /></Pressable>}
                                {details?.label && (
                    ...
                    <TouchableOpacity onPress={() => {
                        ...
                        onClose?.();
                    }} ><CrossIcon size={20} color={getColor("green", 700)} /></TouchableOpacity>
                </View>
            </View>
