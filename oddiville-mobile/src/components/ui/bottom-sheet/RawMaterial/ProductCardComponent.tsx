...
import { setSource, toggleChambers, toggleRawMaterial } from '@/src/redux/slices/bottomsheet/raw-material.slice'
...
const ProductCardComponent: React.FC<ProductCardProps> = ({ data }) => {
    ...
    const {selectedRawMaterials, selectedChambers, source } = useSelector((state: RootState) => state.rawMaterial);
    ...
    const handleToggle = (item: { name: string; description?: string; id?: string } & Record<string, any>) => {
    ...
    return (
        <View style={styles.listContainer}>
            {data?.map((item, index) => {
                ...
                const { image, isRawMaterial } = getImageSource(item.image, source === "chamber" ? false : true);

                return (
                    <Pressable style={styles.card} key={index} onPress={() => handleToggle(item)}>
                        ...
                        <Checkbox checked={isSelected(item.name)} onChange={() => handleToggle(item)} />
                    </Pressable>
                )
            })}
        </View>
