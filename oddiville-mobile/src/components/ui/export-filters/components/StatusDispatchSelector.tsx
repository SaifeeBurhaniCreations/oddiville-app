...
import { DISPATCH_STATUS_OPTIONS } from "@/src/constants/dispatchStatus";
...
const StatusDispatchSelector = ({ state, setState }: FilterComponentProps) => {
  const dispatch = useDispatch();

  const selectedStatuses = useSelector(
    (state: RootState) => state.exportDispatchStatus.selectedDispatchStatuses,
  );
  ...
  const handleSelect = () => {
    dispatch(setSource("export-dispatch-status"));
    validateAndSetData("nothing", "select-dispatch-status");
  };
  ...
  const displayValue = useMemo(() => {
    ...
    const labels = selectedStatuses.map((value) => {
      return (
        DISPATCH_STATUS_OPTIONS.find((s) => s.value === value)?.label ?? value
      );
    });
        ...
        <View style={[styles.chamberCard, styles.borderBottom]} key={status}>
          <View style={styles.Hstack}>
            ...
            <View style={styles.Vstack}>
              <B1>
                {truncate(
                  DISPATCH_STATUS_OPTIONS.find((s) => s.value === status)
                    ?.label ?? status,
                )}
              </B1>
            </View>
...
export default StatusDispatchSelector;
